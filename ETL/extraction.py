...
def getPersonIds():
    return pd.read_json(
        f'http://files.tmdb.org/p/exports/person_ids_{fetchdate}.json.gz',
        lines=True)


def getCompanyIds():
    return pd.read_json(
        f'http://files.tmdb.org/p/exports/production_company_ids_{fetchdate}.json.gz',
        lines=True)


def getKeywordIds():
    return pd.read_json(
        f'http://files.tmdb.org/p/exports/keyword_ids_{fetchdate}.json.gz',
        lines=True)


def getCollectionIds():
    return pd.read_json(
        f'http://files.tmdb.org/p/exports/collection_ids_{fetchdate}.json.gz',
        lines=True)


def getTVSeriesIds():
    return pd.read_json(
        f'http://files.tmdb.org/p/exports/tv_series_ids_{fetchdate}.json.gz',
        lines=True)


def getTVNetworkIds():
    return pd.read_json(
        f'http://files.tmdb.org/p/exports/tv_network_ids_{fetchdate}.json.gz',
        lines=True)
...
def extractAllIDsToDB():
    ...
    if movieids.shape[0] > 0:
        ...
        dataloader.inc_load_with_index(
            df=movieids,
            tbl="MovieIds",
        )
    else:
    ...
    if collectionids.shape[0] > 0:
        print(
            f"Dumping {collectionids.shape[0]} records into Collections table")
        dataloader.inc_load_with_index(
            df=collectionids,
            tbl="Collections",
        )
...
def getAndDumpMoviesRaw():
    ...
    for chunk in np.array_split(ids['id'], len(ids) // 500 + 1):
        ...
        df_movies = df_movies[[
            'adult', 'backdrop_path', 'budget', 'genres', 'homepage', 'id',
        ...
        df_movies['production_companies'] = df_movies[
            'production_companies'].astype('str')
        df_movies['production_countries'] = df_movies[
            'production_countries'].astype('str')
        df_movies['spoken_languages'] = df_movies['spoken_languages'].astype(
            'str')

        df_movies['vote_average'] = pd.to_numeric(df_movies.vote_average,
                                                  errors='coerce')
        ...
        dataloader.inc_load_with_index(df=df_movies,
                                       tbl="MoviesRaw",
                                       fastexecute=False)
        session.close()